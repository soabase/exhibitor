...
public class LogIndexer
{
    ...
    private void indexRecord(TxnHeader header, Record record, AtomicInteger count, AtomicLong from, AtomicLong to) throws IOException
    {
        if ( record instanceof CreateTxn )
        {
            ...
            Document document = makeDocument(header, EntryTypes.CREATE, count, from, to);
            document.add(new Field(FieldNames.EPHEMERAL, createTxn.getEphemeral() ? "1" : "0", Field.Store.YES, Field.Index.NOT_ANALYZED));
    ...
    private Document makeDocument(TxnHeader header, int type, AtomicInteger count, AtomicLong from, AtomicLong to)
    {
        ...
        document.add(new Field(FieldNames.TYPE, Integer.toString(type), Field.Store.YES, Field.Index.NOT_ANALYZED));
